...
![](H:\database学习\database-study\image\what-is-b-plus-tree.webp)



![](.\image\0_-u58IBfWfAz5H_fn.png)
...
谈到 MySQL 的锁，其实就是在谈**“秩序”**。如果数据库是一座图书馆，锁就是管理员，决定谁能进书库、谁能翻书、谁能改写。

MySQL 的锁机制非常复杂，但我们可以从**颗粒度**、**模式**和**策略**三个维度来拆解。
...
- **全局锁 (Global Lock)：** 锁定整个数据库实例。
  - **指令：** `Flush tables with read lock (FTWRL)`。
  - **场景：** 主要用于全库逻辑备份。它会让整个库处于只读状态，业务基本停滞。
- **表级锁 (Table Lock)：** 锁定整张表。
  - **特点：** 开销小，加锁快；但并发度极低。
  - **元数据锁 (MDL)：** 当你对表做增删改查时，会自动加 MDL 读锁；当你修改表结构（加字段）时，会自动加 MDL 写锁。这就是为什么**在大表加字段会导致整个库崩掉**的原因。
- **行级锁 (Row Lock)：** 只锁定某一行。
  - **特点：** 由 **InnoDB** 存储引擎实现。开销大，可能出现死锁；但并发度最高。
  - **注意：** MySQL 的行锁是**索引锁**。如果你更新时没用到索引，它会退化成表锁（全表扫描）。
...
- **共享锁 (S 锁 / 读锁)：** 多个事务可以同时读同一行，但不允许写。
  - `SELECT ... LOCK IN SHARE MODE`。
- **排他锁 (X 锁 / 写锁)：** 一个事务加了写锁，其他事务既不能读也不能写。
  - `SELECT ... FOR UPDATE` 或普通的 `UPDATE/DELETE`。
- **意向锁 (Intention Lock)：** 这是一个“插旗”机制。事务想给某行加锁前，先在表上插个旗子。这样别人想锁整张表时，一眼就能看到表里有人加了行锁，不用一行行去检查。

------

### 3. InnoDB 的“必杀技”：解决幻读的锁

InnoDB 之所以强大，是因为它在 `Repeatable Read` 级别下用了三种特殊的锁：

1. **记录锁 (Record Lock)：** 仅仅锁住索引记录本身。
2. **间隙锁 (Gap Lock)：** 锁住索引记录之间的空隙（不包括记录本身）。目的是防止别人在这些空隙里插入新数据。
3. **临键锁 (Next-Key Lock)：** 记录锁 + 间隙锁。它是 InnoDB 解决**幻读**的核心武器。

------

### 4. 乐观锁 vs 悲观锁 (策略)

这其实是程序员层面的处理思想，而不是 MySQL 数据库自带的功能。

- **悲观锁：** 认为冲突一定会发生。直接 `SELECT ... FOR UPDATE`，我不释放锁，谁也别想碰。
- **乐观锁：** 认为冲突很少发生。不加锁，更新时比对 `version`。正如你之前问的，失败了就重试。
...
**MySQL 的处理：** InnoDB 会自动检测死锁，并强制让其中一个代价较小的事务**回滚**，让另一个成功，从而解开死扣。

------

### 避坑指南：给你的办公系统优化建议

1. **尽量走索引：** `UPDATE` 语句如果没有索引，会把全表锁住，办公系统瞬间变卡。
2. **控制事务大小：** 不要在一个事务里处理太多业务，事务越长，占锁时间越长，越容易死锁。
3. **MDL 锁风险：** 绝对不要在业务高峰期给大表修改字段，那会导致所有访问该表的请求堆积，最终撑爆数据库连接。
...
**定义：** 一个事务读取了另一个事务**尚未提交**（Uncommitted）的数据。

- **场景演示：**
  1. 事务 A 把你的账户余额从 100 元改成了 500 元，但**还没点击“保存（提交）”**。
  2. 这时事务 B 进来查看，发现余额是 500 元，于是开心地去消费了。
  3. 结果事务 A 突然发现操作失误，执行了**回滚（Rollback）**，余额变回了 100 元。
- **后果：** 事务 B 看到的数据其实是虚构的（“脏”的），导致业务逻辑出错。

### 2. 幻读 (Phantom Read)

**定义：** 当一个事务按某个条件**范围**查询数据时，另一个事务插入了新行，导致第一个事务再次查询时，发现了原本不存在的“幻影”记录。

- **场景演示：**
  1. 事务 A 查询“所有年龄大于 20 的员工”，发现有 5 个人。
  2. 这时事务 B 插入了一个新员工，年龄 25 岁，并**提交**了。
  3. 事务 A 再次执行相同的查询，发现变成了 6 个人。
- **注意：** 幻读重点在于**记录数量的变化**（原本没有的现在有了），而不是单条数据内容的修改（那是不可重复读）。
...
| **现象**       | **核心特征**                             | **解决方法 (隔离级别)**                     |
| -------------- | ---------------------------------------- | ------------------------------------------- |
| **脏读**       | 读到了别人**没发货（没提交）**的数据。   | 开启 **Read Committed** (读已提交)          |
| **不可重复读** | 同一行数据，两次读的结果**内容不一样**。 | 开启 **Repeatable Read** (可重复读)         |
| **幻读**       | 同样的范围查询，两次读的**条数不一样**。 | 开启 **Serializable** (串行化) 或使用间隙锁 |

------

### 为什么会出现这些问题？

这其实是数据库在**性能**与**准确性**之间做的权衡。

- **隔离级别越低：** 并发性能极高，但数据像“大杂烩”一样乱。
- **隔离级别越高：** 数据绝对准确，但大家都要排队等待，效率极低。

> **小贴士：** MySQL 的默认隔离级别是 `Repeatable Read`（可重复读），它通过一种叫 MVCC 的技术巧妙地解决了脏读，并能在很大程度上避免幻读。



除了你提到的脏读和幻读，在数据库并发领域，最常被一起讨论的其实是**不可重复读**。此外，还有一个由于隔离级别太高或处理不当引起的**丢失更新**问题。

我们可以把这几种现象看作是并发控制中的“四大金钢”：
...
- **定义：** 在同一个事务内，多次读取**同一条数据**，结果却不一样。

- **场景：** 1.  事务 A 读取了你的工资，显示为 5000 元。

  \2.  这时事务 B 偷偷把你的工资改成了 8000 元，并且**提交**了。

  \3.  事务 A 再次读取你的工资，发现变成了 8000 元。

- **对比：** * **脏读**是读到了“没提交”的垃圾数据。

  - **不可重复读**是读到了“已提交”的修改，但在同一个事务里前后不一致，让人很困惑。

### 2. 丢失更新 (Lost Update)

这是一种比“读”更严重的写冲突问题。

- **定义：** 两个事务同时修改同一行，后提交的事务覆盖了先提交的事务，导致先前的修改“人间蒸发”。

- **场景：** 1.  你和你的朋友同时在抢一张票（库存 1）。

  \2.  事务 A 读取库存为 1，准备下单。

  \3.  事务 B 也读取库存为 1，抢先下单并提交，库存变 0。

  \4.  事务 A 稍后也提交了订单，基于它最初读到的“1”，它把库存更新成了 0。

- **结果：** 事务 B 的操作实际上被事务 A 覆盖了，数据库记录没出问题，但业务逻辑上 B 的努力白费了。
...
| **隔离级别**                    | **脏读** | **不可重复读** | **幻读**    | **性能**    |
| ------------------------------- | -------- | -------------- | ----------- | ----------- |
| **Read Uncommitted** (读未提交) | ❌ 会发生 | ❌ 会发生       | ❌ 会发生    | 极高 (裸奔) |
| **Read Committed** (读已提交)   | ✅ 已解决 | ❌ 会发生       | ❌ 会发生    | 高          |
| **Repeatable Read** (可重复读)  | ✅ 已解决 | ✅ 已解决       | ⚠️ 可能发生* | 中          |
| **Serializable** (串行化)       | ✅ 已解决 | ✅ 已解决       | ✅ 已解决    | 低 (排队)   |

> **注：** 在 MySQL 的 InnoDB 引擎中，`Repeatable Read` 级别下通过 **Next-Key Locks（间隙锁）** 已经很大程度上解决了幻读问题。

------

### 补充：快照读 vs 当前读

在现代数据库（如 MySQL）中，为了平衡性能，还区分了两种“读”的模式：

- **快照读 (Snapshot Read)：** 简单的 `SELECT`。它读的是数据的历史版本（快照），不需要加锁，所以性能极高，不会被别人阻塞。
- **当前读 (Current Read)：** 比如 `SELECT ... FOR UPDATE` 或 `UPDATE/INSERT`。它必须读取数据库中最新的、被提交的数据，并且会对读取的记录加锁。

**你想深入了解一下 MySQL 是如何通过“MVCC（多版本并发控制）”来实现这些隔离级别而不卡顿的吗？**



没问题，刚才那个例子可能有点绕。我们换一个更生活化的场景：**“多人共同编辑一张 Excel 表格”**。

想象你和同事都在改同一份名为 `库存统计.xlsx` 的在线文档，目前 **“A 产品的库存”** 是 **100**。

------

### 场景：两个老板同时“补货”

1. **老板 A 进场：** 他打开表格，看到库存是 **100**。他去仓库搬了 **50** 个货，准备把数字改成 **150**。
2. **老板 B 进场（同时）：** 他也打开了表格，因为老板 A 还没点击“保存”，所以老板 B 看到的库存也是 **100**。他也去搬了 **30** 个货，准备把数字改成 **130**。
3. **老板 A 动作快：** 他先把数字改成了 **150**，点击了 **“保存并提交”**。现在的数据库里是 150。
4. **老板 B 紧随其后：** 他把自己算好的 **130** 填进去，也点击了 **“保存并提交”**。
...
- **实际上：** 100 + 50 + 30 = **180**。
- **发生了什么：** 老板 A 辛苦搬的 50 个货，在数据库里被老板 B 的操作直接**覆盖（抹掉）**了。

**这就是“丢失更新”：** 两个人都基于旧数据做计算，后提交的人把先提交的人的成果给弄丢了。
...
#### 1. 悲观锁 (Pessimistic Locking) —— “先占坑，后干活”

老板 A 打开表格时，直接把这行数据**锁死**。老板 B 想看的时候，电脑提示：“该条目正在被编辑，请稍后”。

- **SQL 表现：** `SELECT ... FOR UPDATE`。
- **优点：** 绝对安全。
- **缺点：** 效率低，别人得干等着。

#### 2. 乐观锁 (Optimistic Locking) —— “提交前，比对版本”

我们在表格里加一列 **version（版本号）**。

1. 老板 A 读到的是 `库存=100, version=1`。
2. 老板 B 读到的也是 `库存=100, version=1`。
3. 老板 A 提交时，系统检查：当前数据库里还是 `version=1` 吗？是的。于是更新为 `150`，并将 `version` 改为 `2`。
4. 老板 B 提交时，系统检查：当前数据库里还是 `version=1` 吗？**不是了，已经变 2 了！**
5. **系统报错：** “数据已被他人修改，请刷新后重试”。老板 B 只能重新读取 150，再加 30，最后变成 180。

------

**总结一下：**

- **脏读/不可重复读/幻读：** 是你“看”到了不该看的东西。
- **丢失更新：** 是你“写”的时候把别人的努力给覆盖了。



### 不可重复读 场景 还是感觉很没必要

你的直觉非常敏锐。在很多简单的互联网业务（比如刷朋友圈、看商品评论）中，数据变了就变了，看到最新的确实没问题，甚至更好。

之所以觉得“没必要”，是因为你还没遇到那些对**数据一致性**要求极高的**“长事务”**场景。在某些特定逻辑下，如果数据在处理中途变了，整个程序会直接崩溃或产生财务灾难。

我给你举两个必须拦截“不可重复读”的**硬核场景**：

------

### 1. 银行日终批处理（对账报表）

想象银行后台运行一个自动程序，生成今天的财务报表。

- **步骤 A：** 程序读取账户余额：$10,000$ 元。
- **步骤 B：** 程序根据这 $10,000$ 元去计算该付给客户的利息（假设 $100$ 元）。
- **步骤 C：** 程序再次读取余额，准备把“余额 + 利息”写回数据库。

**如果允许“不可重复读”：**

在步骤 B 的时候，客户取走了 $5,000$ 元并提交。到了步骤 C，程序读到余额变成了 $5,000$。此时程序就会把 $5,000 + 100$ 写回去。

**结果：** 客户平白无故丢了 $4,900$ 元。会计平账时，发现天塌了。

> **核心点：** 程序逻辑是基于“第一眼看到的那个数字”展开的，如果数字中途变了，后续所有基于此的推导都是错的。
...
1. **校验阶段：** 查询订单金额是否满 **100** 元（看到是 **105** 元，符合条件）。
2. **执行阶段：** 调用减免接口，准备扣减 **20** 元。

**如果允许“不可重复读”：**

在步骤 1 和 2 之间，你突然在另一个页面点击了“取消订单中某个商品”，订单金额瞬间变成了 **50** 元并提交。

如果步骤 2 再次读取订单金额发现是 **50**，却依然执行了“减 20”的逻辑。
...
1. 你先查财务部，总和 **50 万**。
2. 这时，有个员工从**技术部**调到了**财务部**，工资 **3 万**。他的操作修改了两个部门的数据并提交。
3. 你接着查技术部，因为他调走了，技术部总额少了 **3 万**。

**如果不开启“可重复读”：**

你算出的全公司总额会平白无故**少了 3 万**（因为查财务部时他还没来，查技术部时他已经走了）。
...
| **业务类型**       | **对“不可重复读”的态度** | **原因**                                           |
| ------------------ | ------------------------ | -------------------------------------------------- |
| **社交/咨询类**    | 没必要解决               | 用户看到最新的状态更好，追求性能。                 |
| **金融/审计/报表** | **必须解决**             | 必须保证整个任务周期内，数据像“定格照片”一样静止。 |
| **复杂逻辑判断**   | **必须解决**             | 防止“校验时合法，执行时已非法”的逻辑漏洞。         |

**一句话总结：**

“可重复读”是为了给程序提供一个**稳定的运行环境**。如果你只是查个数据给用户看，确实没必要；但如果你要拿这个数据去算钱、去写回数据库，那它就是救命稻草。

既然你提到了没必要，你是觉得开启这个级别会严重影响性能吗？我们可以聊聊 MySQL 是怎么做到**不加锁也能实现“可重复读”**的（即 MVCC 机制）。



### 优惠券核销 这个例子中的问题怎么解决呢

要解决优惠券核销中“校验时合法，执行时已非法”的问题，核心思路是保证**“校验+执行”这两个步骤的原子性和一致性**。

在实际开发中，通常有以下三种主流解决方案：

### 1. 数据库层面：提高隔离级别 (Repeatable Read)

这是最简单的方案。如果你使用的是 MySQL（默认就是 `Repeatable Read`），在同一个事务中，你第二次读取到的金额依然会是 **105元**。

- **逻辑：** 虽然用户在另一个页面改成了 50 元，但你的核销事务里看到的还是 105 元镜像。
- **弊端：** 这种方式虽然保证了“不可重复读”，但会引发**丢失更新**。因为你基于 105 元计算并写回，会把用户修改后的 50 元给覆盖掉。所以，**光靠隔离级别是不够的**。

------

### 2. 技术层面：使用“当前读”加锁 (悲观锁)

这是最稳妥的解决办法。在校验阶段，我不只是简单的 `SELECT`，而是使用 `SELECT ... FOR UPDATE`。

- **操作流程：**
  1. **核销事务开启**。
  2. 执行 `SELECT amount FROM orders WHERE id = 1 FOR UPDATE;`
  3. **效果：** 这行数据被你“锁住”了。此时用户如果想在另一个页面修改订单金额，他的修改操作会**阻塞等待**，直到你的核销事务完成。
  4. 你校验金额是 105 元，执行减免，提交事务。
  5. 用户那边的修改操作才会被执行（此时他会发现金额已经变了）。
- **适用场景：** 对钱、库存等核心数据操作，必须保证绝对准确。

------

### 3. 业务层面：最终校验 + 扣减 SQL (推荐方案)

在 Java 代码里判断是不够安全的，我们应该把**关键的判断逻辑下沉到 SQL 语句中**。

这种方案结合了**乐观锁**的思想，不需要长时间锁表，性能最高。
...
- **Java 代码逻辑：**
  ...
  if (order.getAmount().compareTo(new BigDecimal("100")) < 0) {
      throw new BizException("不满100元，无法使用此券");
  }
  
  // 3. 执行扣减（关键：在更新 SQL 中再次校验条件）
  int rows = orderMapper.reduceAmount(orderId, couponDiscount, 100);
  if (rows == 0) {
...
- **对应的 SQL：**
  ...
  <update id="reduceAmount">
      ...
      SET amount = amount - #{discount},
          version = version + 1
      WHERE id = #{id} 
        AND amount >= #{threshold}  -- 核心：再次检查是否满足 100 元的门槛
        AND status = 'PENDING'      -- 核心：确保订单状态还是待支付
  </update>
  ```
...
1. **如果你追求极致安全（如银行、大额转账）：** 用 **方案2 (FOR UPDATE)**。虽然慢一点，但绝对不会出乱子。
2. **如果你追求高并发性能（如电商秒杀、抢券）：** 用 **方案3 (SQL 条件校验)**。这种方式只在更新那一刻判断，不会长时间阻塞别人读取数据。

**为什么你之前觉得“不可重复读”没必要？**

是因为在旧的编程思维里，我们习惯把数据查出来，在 Java 内存里算好，再写回去。但在高并发下，**“在 SQL 里进行条件更新”** 才是最核心的解决之道，它避开了“读”到什么的问题，直接在“写”的时候守住最后一道关卡。



### 使用乐观锁, 那岂不是又要算一遍吗

你的直觉非常敏锐！这正是**乐观锁**被很多人“吐槽”的地方：**如果并发很高，大家都在抢着改，就会导致大量的更新失败，然后被迫不断地“重试”。**

这种“又要算一遍”的行为，在技术上叫做**“自旋（Spin）”**或**“重试逻辑”**。
...
当冲突真的发生时（版本号变了），为了保证数据的准确性，你只能：

1. **丢弃**刚才算错的结果（因为那是基于旧数据算的）。
2. **重新读取**数据库里最新的值。
3. **重新计算**业务逻辑。
4. **再次尝试**提交。
...
- **对比悲观锁 (FOR UPDATE)：**
  - **悲观锁：** 像排队上厕所，只有一个人能进去，后面的人全都**阻塞（Block）**在那。阻塞会导致线程挂起、上下文切换，这在操作系统层面是非常“重”的操作。
  - **乐观锁：** 像大家都在写文档，写完提交时发现冲突了才报错。虽然要重写，但在没有冲突时，大家都是**并行**的，速度极快。
- **结论：** * 如果**冲突很少**（比如 100 次操作才 1 次冲突），乐观锁性能完胜。
  - 如果**冲突极多**（比如秒杀 1 件商品有 1 万人抢），乐观锁会导致 CPU 全在忙着“重试”，这时候**悲观锁**反而更省心。
...
我们将逻辑**从 Java 内存下沉到 SQL 语句中**。利用数据库自带的行级锁，让数据库帮你算。

#### 方案 A：直接在 SQL 里计算（原子更新）

不要在 Java 里算 `newCount = oldCount + 1`，直接把计算公式写进 SQL。
...
```
-- 这种写法不需要 version，也不需要重试
UPDATE t_stock 
SET count = count - 1 
WHERE id = 1 AND count > 0;
```

- **优点：** 数据库会自动保证这一行在执行减法时是安全的。即使 100 个请求同时来，数据库也会排队执行，每个请求都能成功，且不需要 Java 重试。

#### 方案 B：状态机更新

适用于订单状态流转（比如：待支付 -> 已支付）。
...
WHERE id = 1 AND status = 'PENDING'; -- 只有是待支付时才更新
```

- **优点：** 只要返回 `rows > 0` 就是成功，返回 `0` 说明别人已经改过了，直接告诉用户“支付成功”或“状态异常”即可，通常不需要重试。

------

### 4. 什么时候必须“再算一遍”？

如果你的业务逻辑**极其复杂**，SQL 根本写不下（比如：要根据库存量、用户等级、活动规则、优惠券金额等多重因素综合计算出一个最终价格），这种情况下：

1. SQL 没法直接算。
2. 你只能把数据拿回 Java。
3. 如果更新失败，**确实必须重新读取、重新计算。**

------

### 总结建议

- **简单加减：** 永远优先使用 `SET count = count + x`，不要用乐观锁重试。
- **状态切换：** 优先使用 `WHERE status = '旧状态'`。
- **复杂逻辑且并发不高：** 使用乐观锁 + `while` 循环重试。
- **复杂逻辑且并发极高：** 放弃乐观锁，改用 **Redis 分布式锁** 或 **悲观锁**，让大家老老实实排队。



### 但是我感觉很多办公系统里, 根本没这么多锁机制

你的感觉非常准。在很多**内部办公系统（OA）、ERP、CMS（后台管理系统）**中，确实很少看到代码里写满各式各样的锁。

这不是因为程序员偷懒，而是基于**“投入产出比”**和**“业务真实场景”**做出的选择。主要有以下三个原因：

------

### 1. 并没有那么多“并发”

在办公系统里，最大的并发往往不是来自“用户抢数据”，而是来自“用户抢厕所”。

- **电商系统：** 10万人在同一秒抢100件衣服，这是高并发，必须加锁。
- **办公系统：** 一个部门就5个人，能同时修改同一个报表的概率比中彩票还低。
- **结论：** 既然冲突几乎不发生，为了那万分之一的概率去写复杂的锁逻辑、加重数据库负担，反而是一种**过度设计**。

### 2. “最后一次提交获胜”也是一种策略

很多办公系统默认采用一种叫 **“Last-Write Wins”（最后一次更新有效）** 的简单粗暴策略。

- 如果王经理和张会计同时改一个备注，王经理先点保存，张会计后点保存。
- 结果：张会计的内容覆盖了王经理的。
- **在办公环境下：** 这通常不是什么大问题，王经理发现没改成功，骂一句“怎么没存上”，再改一遍就行了。

### 3. 使用了隐形的“业务锁”

很多办公系统不使用“技术锁”（SQL 锁），而是用**“业务流程”**把并发扼杀在了摇篮里：

- **编辑权锁定：** 比如你在修改某条公文时，系统直接显示“王经理正在编辑中...”，你只能看，不能点保存。
- **状态机控制：** 只有处于“草稿”状态的单子才能改。一旦有人点了“提交审批”，单子状态变了，其他人再点保存时，后台 SQL 的 `WHERE status = 'DRAFT'` 就失效了。
- **工单制：** 任务是分派给个人的。你处理你的，我处理我的，我们根本没有机会同时操作同一条数据。

------

### 什么时候办公系统“不得不”加锁？

虽然平时不怎么用，但只要涉及到**“钱”**和**“核心资源”**，办公系统也得变严肃：

| **场景**           | **是否加锁** | **为什么**                                               |
| ------------------ | ------------ | -------------------------------------------------------- |
| **修改员工姓名**   | 不加锁       | 覆盖了就覆盖了，没啥损失。                               |
| **员工请假扣余额** | **必须加锁** | 两个人同时帮他请假，余额不能扣成负数。                   |
| **会议室预订**     | **必须加锁** | 否则会导致一个房间同时进去两波人开会，那是大型尴尬现场。 |
| **财务报表结账**   | **必须加锁** | 结账时不能允许有人再录入新的凭证。                       |

------

### 总结

你之所以感觉不到锁，是因为大部分办公系统的逻辑是**“读多写少”**且**“任务隔离”**的。

**一个有趣的现象：**

很多老旧的办公系统，如果真的发生了“丢失更新”，它们甚至不报错，而是直接让数据乱掉。直到某天用户投诉“我填的数据怎么没了”，程序员才会上线去加一个 `version` 字段（乐观锁）。

你现在负责的系统是哪一类？如果是处理**审批流**或者**库存管理**，我建议你至少加一个**乐观锁**（版本号），这是最性价比的保命手段。

既然聊到了办公系统，你想看看在 Spring Boot 里如何通过一行代码（`@Version`）给你的办公软件加上最基础的防线吗？
